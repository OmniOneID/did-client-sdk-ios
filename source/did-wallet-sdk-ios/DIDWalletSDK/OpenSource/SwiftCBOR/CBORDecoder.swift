...
extension CBOR {
    static func decode(_ input: [UInt8], options: CBOROptions = CBOROptions()) throws -> CBOR? {
...
class CBORDecoder {
    ...
    var options: CBOROptions
    private var currentDepth = 0

    init(stream: CBORInputStream, options: CBOROptions = CBOROptions()) {
        self.istream = stream
        self.options = options
    }

    init(input: ArraySlice<UInt8>, options: CBOROptions = CBOROptions()) {
        self.istream = ArraySliceUInt8(slice: input)
        self.options = options
    }

    init(input: [UInt8], options: CBOROptions = CBOROptions()) {
